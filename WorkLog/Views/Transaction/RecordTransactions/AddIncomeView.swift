    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Button {
                    viewModel.addTransaction(description: incomeDescription, origin: incomeOrigin, date: incomeDate, amount: incomeAmount, isExpense: false, formattedCurrency: viewModel.formatCurrency(incomeAmount))
                    resetFields()
                } label: {
                    Text("Add Income")
                }
